function [ d,h,t ] = hyperb_ev_tho(dt,f0,tmax,h,tau,v,amp,snr,L);
%HYPERB_EV_THO 
%A program to generate thonia's first data containing hyperbolas.
%
...
%v6=
vrms1=1500
vrms2=sqrt((1500^2*0.8+2000^2*0.1)/0.9)
vrms3=sqrt((1500^2*0.8+2000^2*0.05*2+2600^2*0.1*2)/1.1)
vrms4=sqrt((1500^2*0.8+2000^2*0.05*2+2600^2*0.1*2+3500^2*0.2)/1.3)
vrms5=sqrt((1500^2*0.8+2000^2*0.05*2+2600^2*0.1*2+3500^2*0.2+5000^2*0.1)/1.4)
vrms6=sqrt((1500^2*0.8+2000^2*0.05*2+2600^2*0.1*2+3500^2*0.2+5000^2*0.1+4400^2*0.1)/1.5)
 if nargin == 0
  dt = 2./1000;
  tmax = 1.8;
  h = [0:20:1000];
  tau = [2*0.4,2*.45,2*0.55,2*0.65,2*0.7,2*0.75];
  v = [vrms1,vrms2,vrms3,vrms4,vrms5,vrms6];
  amp = [1., 1., 1., 1,-1,1];
  f0 = 40;
  snr = 1000 ;
  L = 1;
