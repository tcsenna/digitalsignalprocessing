...
%  [e0,h0]=fep_l1l2_pso(d,N,500,200,200,1);
 ...
 h0=zeros(1,N);
 h0(ceil(N/2))=1;
 [e2,h2]=fep_wiggins(d,h0,1000,0.5,1,0);
 
 [h0] = bestwiener(w',45);
 [best1] = bestwiener(h1,45);
 [best2] = bestwiener(h2,45);
...
% Estimate the reflectivity using l1 (sparse) regularization
 ...
 subplot(131); wigb(e2,2,cdp,t_axis);
 xlabel('cdp');
 ylabel('Time (s)');
