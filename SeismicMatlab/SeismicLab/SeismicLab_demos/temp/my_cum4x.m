function   y_cum = my_cum4x (w,x,y,z, maxlag, nsamp, overlap, flag, k1, k2)
%
%	y_cum = my_cum4x (w,x,y,z, maxlag, samp_seg, overlap, flag, k1, k2)
%
...
% ----------- estimate second- and fourth-order moments; combine ------
 ...
 for i=1:nrecs
     ...
     ys = y(ind);  ys = ys(:) - mean(ys);  ys = conj(ys);
...
%                     create the "IV" matrix: offset for second lag
     ...
     if (k1 >= 0)
 	ziv(1:nsamp-k1)  = ws(1:nsamp-k1) .* ys(k1+1: nsamp);
	R_wy = R_wy + ws(1:nsamp-k1)' * ys(k1+1:nsamp);
     else
       	ziv(-k1+1:nsamp) = ws(-k1+1:nsamp)  .* ys(1:nsamp+k1);
...
%                     create the "IV" matrix: offset for third lag
	...
	M_wz = M_wz + ws(1:nsamp-k2).' * zs(k2+1:nsamp);
        ...
        ziv(1:-k2)    = zeros(-k2,1);
	M_wz = M_wz + ws(-k2+1:nsamp).' * zs(1:nsamp+k2); %in loc de 1:nsamp-k2 am pus +k2
     end
     ...
     R_wx = my_cum2x(ws,      xs, maxlag,         nsamp, overlap0, flag);
     R_zx = my_cum2x(zs,      xs, maxlag+abs(k2), nsamp, overlap0, flag);
     M_yx = my_cum2x(conj(ys),xs, maxlag+abs(k1), nsamp, overlap0, flag);
